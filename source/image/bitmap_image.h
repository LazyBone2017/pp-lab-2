...
public:
    ...
    [[nodiscard]] BitmapImage transpose_parallel(int thread_count = 1) const;

	[[nodiscard]] BitmapImage get_grayscale() const;

    [[nodiscard]] BitmapImage get_grayscale_parallel(int thread_count = 1) const;

	void mystery();

    void mystery_parallel(int thread_count = 1);
